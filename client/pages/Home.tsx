    ...
    try {
      ...
      if (address.trim()) {
        if (language === "evm") {
          tasks.push(
            fetch(
              `/api/daemon/address/${encodeURIComponent(address)}/risk-score`,
            ).then((r) => r.text()),
          );
          tasks.push(
            fetch(`/api/daemon/eth/txlist/${encodeURIComponent(address)}`).then(
              (r) => r.text(),
            ),
          );
          tasks.push(
            fetch(`/api/daemon/analyze/${encodeURIComponent(address)}`).then(
              async (r) => {
          ...
          tasks.push(
            fetch(
              `/api/daemon/sol/address/${encodeURIComponent(address)}/transactions`,
            ).then((r) => r.text()),
          );
          tasks.push(
            fetch(
              `/api/daemon/sol/address/${encodeURIComponent(address)}/balance-changes`,
            ).then((r) => r.text()),
          );
          tasks.push(
            fetch(
              `/api/daemon/sol/address/${encodeURIComponent(address)}/signatures`,
            ).then((r) => r.text()),
          );
        }
      }
        ...
        <main className="min-h-screen">
          ...
          <section className="px-4 py-8 md:px-8">
            ...
            <div className="mt-6 rounded-2xl border bg-card p-4">
              ...
              <pre className="max-h-[50vh] overflow-auto whitespace-pre-wrap break-words text-sm">
                {result}
              </pre>
            </div>
          </section>
